...
class AgentVPartial(Agent):
    def move_agent(self, prey_transition_matrix, predator_transition_matrix, node_distances, n_network):
        count = 0
        belief = [1 / 49] * 50
    ...
    def get_next_move(self, belief, prey_transition_matrix, predator_transition_matrix, node_distances, n_network):
        upartial = {}
                ...
                for neighbour_of_predator in neighbours_of_predator:
                    for i in range(len(belief)):
                        if self.utility[possible_next_move, neighbour_of_predator, i, 1] == np.inf and \
                            ...
                            # X = np.zeros((4, 1))
                            # X[0, 0] = possible_next_move
                            # X[1, 0] = neighbour_of_predator
                            # X[2, 0] = np.dot(belief, node_distances[self.currPos])
                            # X[3, 0] = np.dot(belief, node_distances[self.predator.currPos])
                            X = [
                                possible_next_move,
                                neighbour_of_predator,
                                np.dot(belief, node_distances[self.currPos]),
                                np.dot(belief, node_distances[self.predator.currPos]),
                                *belief
                            ]
                            X = np.array(X).reshape((len(X), 1))
                            predicted_utility = n_network.predict(X)
                            summation += (predator_transition[self.predator.currPos, neighbour_of_predator] *
                                          belief_with_transition[i] * predicted_utility)
